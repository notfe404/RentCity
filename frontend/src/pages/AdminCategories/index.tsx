  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!cancelled) toast.error('Không tải được danh mục xe');
      } finally {
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-[1.5rem] border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-5 border-t border-gray-100">
            <p className="text-xs font-bold text-gray-400">Tổng: {filtered.length} danh mục</p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h2 className="text-lg font-black text-gray-900">{editing ? 'Sửa danh mục' : 'Thêm danh mục mới'}</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1.5">Tên danh mục *</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-500 mb-1.5">Tỷ lệ cọc (0.1 - 1.0)</label>
                  <input
                    type="number"
                    min={0}
                    max={1}
