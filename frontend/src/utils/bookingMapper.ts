...
export const DEPOSIT_STATUS_META: Record<ApiDepositStatus, { label: string; color: string }> = {
  UNPAID: { label: 'Chưa thanh toán cọc', color: 'text-yellow-500' },
  PAID: { label: 'Đã xác nhận cọc', color: 'text-[#78ad44]' },
  FORFEITED: { label: 'Mất cọc', color: 'text-red-500' },
  REFUNDED: { label: 'Đã hoàn cọc', color: 'text-gray-500' },
  NOT_REQUIRED: { label: 'Không yêu cầu cọc', color: 'text-gray-500' },
};
